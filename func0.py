...
def calcUnitsSA( molC, N, chrend, chrstart, res, bond_length, eta=.05)   :
    # =============================================================================
    # Unit of measure
    # =============================================================================
    # pdb.set_trace()
    ...
    # linearly depends on the estimation of nucleus radius!!!
    sig = ( 1/ N ) ** ( 1/ 3.) * b  # meters
    # sig = ( res / ll * (ll/res / N) ) ** ( 1/ 3.) * b  # meters
    # sig = ( res / G ) ** ( 1/ 3.) * R  # meters
    dunit = sig / bond_length  
...
def readSV( model ):
    ...
    for chrid, (chrname, chrstart, chrend) in enumerate(chrL):
        ...
        ###
        Pimap = pd.DataFrame([])
        for chrsv in model.chrSV:
            # Pimap = pd.DataFrame( np.repeat( np.linspace( chrstart, chrend-model.bpres, int((chrend - chrstart) / model.bpres) ), model.R) )

            Pimaptmp = pd.DataFrame( np.repeat( np.linspace( 
                chrsv[1], chrsv[2]-model.bpres, int((chrsv[2] - chrsv[1]) / model.bpres) ), model.R) )
            Pimaptmp['chr'] = chrsv[0]
            Pimap = Pimap.append(Pimaptmp)

        if 'svprojection' in dir(model):
            model.randsvid = np.random.randint(10000) * int(model.procid) ##########
            print(model.randsvid)
            ttcselsv = np.int_(
                model.svprojection[chrid].sample(1, axis=0, random_state=model.randsvid).values.flatten())
            
            ttcsel = np.concatenate( [ttcsel, ttcselsv])



        Pimap.columns = ['chrstart','chr']
        Pimap['bs'] = ttcsel
        Pimap['Idx'] = np.array( list(range(0,model.R)) * int(len(ttcsel)/model.R) )
        #
...
def createAnnot( model, param, mod ):
    ...
    # pdb.set_trace()
    # annotIndx.Index = annotIndx.Index * model.bpres / 1000
    if re.match( '.*Real.*', model.modelods_paramsheet ) and model.dirflag:
        ctcfdixn = np.isin( annotIndx.annot , ['CTCF','CTCF+G']) & np.isin( annotIndx.strand , ['-'])
        ctcfdixp = np.isin( annotIndx.annot , ['CTCF','CTCF+G']) & np.isin( annotIndx.strand , ['+'])
        ctcfehdixn = np.isin( annotIndx.annot , ['CTCF+EH']) & np.isin( annotIndx.strand , ['-.'])
        ctcfehdixp = np.isin( annotIndx.annot , ['CTCF+EH']) & np.isin( annotIndx.strand , ['+.'])
    elif re.match( '.*Art.*', model.modelods_paramsheet ) and model.dirflag:
        ctcfdixn = np.isin( annotIndx.annot , ['CTCF','CTCF+G']) & np.isin( annotIndx.strand , ['+'])
        ctcfdixp = np.isin( annotIndx.annot , ['CTCF','CTCF+G']) & np.isin( annotIndx.strand , ['-'])
        ctcfehdixn = np.isin( annotIndx.annot , ['CTCF+EH']) & np.isin( annotIndx.strand , ['+.'])
        ctcfehdixp = np.isin( annotIndx.annot , ['CTCF+EH']) & np.isin( annotIndx.strand , ['-.'])
    ...
    enhdix = np.isin( annotIndx.annot , ['ENHANCER','ENHANCER+G'])
    tesdixp = np.isin( annotIndx.annot , ['TES']) & np.isin( annotIndx.strand , ['+'])
    tesdixn = np.isin( annotIndx.annot , ['TES']) & np.isin( annotIndx.strand , ['-'])
    ...
    annotIndx['annot2'][ctcfehdixp] = [ '>'+str(anncoo) for anncoo in  range(annotIndx[ctcfehdixp].shape[0]) ] 
    annotIndx['annot2'][ctcfehdixn] = [ '<'+str(anncoo) for anncoo in  range(annotIndx[ctcfehdixn].shape[0]) ] 
    ...
    annotIndx['annot2'][tesdixp] = [ '+|'+str(anncoo) for anncoo in  range(annotIndx[tesdixp].shape[0]) ] 
    annotIndx['annot2'][tesdixn] = [ '|-'+str(anncoo) for anncoo in  range(annotIndx[tesdixn].shape[0]) ] 
    ...
    if mod == 'cmbin':
        ...
        annotIndx.tickLabel[ ctcfehdixp] = '>\n^'
        annotIndx.tickLabel[ ctcfehdixn] = '<\n^'
        annotIndx.tickLabel[ promdixp] = '<'
        annotIndx.tickLabel[ promdixn] = '>'
        annotIndx.tickLabel[ enhdix] = '^'
        annotIndx.tickLabel[ tesdixp] = '+|'
        annotIndx.tickLabel[ tesdixn] = '|-'
    else:        
        annotIndx['tickLabel'] = 'NaN'
        annotIndx.tickLabel[ ctcfdixp] = '\n>'
        annotIndx.tickLabel[ ctcfdixn] = '\n<'
...
# =============================================================================
#     annotIndxYaxis = annotIndx.copy() 
#     annotIndxYaxis['tickLabel'] = 'NaN'
#     annotIndxYaxis.tickLabel[ ctcfdixp] = 'v  '
#     annotIndxYaxis.tickLabel[ ctcfdixn] = '^  '
#     annotIndxYaxis.tickLabel[ promdixp] = '^'
#     annotIndxYaxis.tickLabel[ promdixn] = 'v'
#     annotIndxYaxis.tickLabel[ enhdix] = '>'
#     annotIndxYaxis.tickLabel[ tesdixp] = '_'
#     annotIndxYaxis.tickLabel[ tesdixn] = '_'
# 
#     annotIndx2 = annotIndx[annotIndx.tickLabel != 'NaN']    
#     annotIndxYaxis = annotIndxYaxis[annotIndxYaxis.tickLabel != 'NaN']   
# =============================================================================
    ...
    try:
        ...
        chrArt2 = chrArt[(~chrArt['chrStart from bed'].isna()) | (~chrArt['fish'].isna())][['chromStart','chrStart from bed','probe name','fish']]
        # chrArt2.iloc[[0,-1]]
        ...
        specAnnotIndex = param.specAnnotIndex # [295,296,297,298,300,302,405,406,407,409,410]
        annotIndx['annot4'] = annotIndx['probe name']
        annotIndx[annotIndx['annot2']!='int'][['annot2','Index','annot3']]
    
        annomask2 = np.isin( annotIndx.Index, specAnnotIndex)
        annotIndx['annot4'][annomask2] = annotIndx[annomask2].annot2        


    except:
        annotIndx['annot2'] = np.nan
        annotIndx['annot3'] = np.nan
        annotIndx['annot4'] = np.nan
